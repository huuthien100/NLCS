    ...
    function deleteProductImagesFromDirectory($product_img)
    {
        try {
            ...
            echo "Error deleting product images from directory: " . $e->getMessage();
        }
    }
    ...
    if (isset($_POST['delete_product'])) {
        try {
            ...
            $deleteProductDetailSql = "DELETE FROM product_detail WHERE id_product = :product_id";
            $deleteProductDetailStmt = $pdo->prepare($deleteProductDetailSql);
            $deleteProductDetailStmt->bindParam(':product_id', $product_id, PDO::PARAM_INT);
            $deleteProductDetailStmt->execute();

            $deleteProductImgSql = "DELETE FROM product_img WHERE id_product = :product_id";
            $deleteProductImgStmt = $pdo->prepare($deleteProductImgSql);
            $deleteProductImgStmt->bindParam(':product_id', $product_id, PDO::PARAM_INT);
            $deleteProductImgStmt->execute();

            $getImagePathSql = "SELECT product_img FROM products WHERE id_product = :product_id";
            $getImagePathStmt = $pdo->prepare($getImagePathSql);
            $getImagePathStmt->bindParam(':product_id', $product_id, PDO::PARAM_INT);
            $getImagePathStmt->execute();
            $productImagePath = $getImagePathStmt->fetchColumn();

            deleteProductImagesFromDirectory($productImagePath);
    ...
    if (isset($_POST['search'])) {
        ...
        $searchSql = "SELECT p.id_product, p.product_name, p.product_img, p.product_price, p.stock_quantity, c.name_category
                        FROM products p INNER JOIN category c ON p.id_category = c.id_category
    ...
    try {
        ...
        $sql = "SELECT p.id_product, p.product_name, p.product_img, p.product_price, p.stock_quantity, c.name_category FROM products p
